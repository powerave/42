...
import { disconnectDB } from "./app.js";
...
// charging secrets
const tlsOptions = {
	key: fs.readFileSync('/run/secrets/service_users_key'),
	cert: fs.readFileSync('/run/secrets/service_users_crt'),
...
server.on('listening', () => {
  ...
  console.log('Service users listening on ' + bind);
});
...
const gracefulShutdown = async (signal) => {
    ...
    server.close(async () => {
        ...
        try {
            await disconnectDB();
                        console.log('users-db connection closed.');
